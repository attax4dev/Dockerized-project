...
export default function Provider() {
    ...
    useEffect(() => {
        ...
        async function getProvider() {
            try {
                const response = await axios.get('http://localhost:8081/api/user/list-User');
                        ...
                        (providers) ? (
                            <TableContainer >
                                <Table size='small' aria-label="simple table">
                                    ...
                                    <TableBody>
                                        {providers.map(prov => (
                                             (prov.ROLE.name == "ROLE_PROVIDER") ? (
                                                <TableRow key={prov.id}>
                                                    <TableCell component="th" scope="row">{prov.id}</TableCell>
                                                    <TableCell align="center">{prov.name}</TableCell>
                                                    <TableCell align="center">{prov.username}</TableCell>
                                                    <TableCell align="center">{prov.email}</TableCell>
                                                    {(prov.service) ? (
                                                        <TableCell align="center">{prov.service.nom}</TableCell>

                                                    ) : (<TableCell align="center">vide</TableCell>
                                                    )}
                                                    <TableCell align="center">{prov.number}</TableCell>
                ...
                <Grid item >
                    {
                        ...
                        //                                 <TableCell align="center">{med.service.nom}</TableCell>
                        //                                 <TableCell align="center">{med.number}</TableCell>
