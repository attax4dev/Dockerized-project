...
export default function Speciality() {
  ...
  useEffect(() => {
    ...
    async function getCity() {
      try {
        const response = await axios.get('http://localhost:8081/api/service/list-Service');
                                ...
                                <TableBody>
                                    {Specialitys.map(spec =>
                                        <TableRow key={spec.id}>
                                            <TableCell component="th" scope="row">{spec.id}</TableCell>
                                            <TableCell align="right">{spec.nom}</TableCell>
                                            {/* <TableCell align="right">  <SupprimeSpecialite id={spec.id} libelle={spec.libelle}/> </TableCell> */}
                                            <TableCell align="right">  <EditSpeciality id={spec.id} libelle={spec.nom} /> </TableCell>
